...
const addReadmes = fileList => {
  ...
  const addReadmeForDir = dir => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const filesInDir = fileUtils
        .dirFiles(dir, realFiles)
        .filter(file => fileUtils.isMarkdown(file));
  ...
  return Promise.all(
    dirList.map(addReadmeForDir).concat(realFiles.map(file => Promise.resolve(file)))
  );
};
